...
<!---->
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-xl-7 col-lg-8 col-md-9 col-11 text-center">
            <h3>Cadastro emprestimos</h3>
            <p class="blue-text">Cadastre os emprestimos no formulário abaixo</p>
            <div class="card">
                <h5 class="text-center mb-4">Preencha os campos a seguir</h5>
                <form method="POST" action="guardar_emprestimo.php" class="form-card">
                    ...
                    <div class="row justify-content-between text-left">
                        <div class="form-group col-sm-6 flex-column d-flex"> 
                            ...
                            <select name="nm_titulo" class="input"  required>
                            <option value="">Selecione um livro</option>
                               <?php
                                $query = $conn->query("SELECT cd_livro, nm_titulo FROM tb_livro ORDER BY nm_titulo ASC");
                                $registros = $query->fetchAll(PDO::FETCH_ASSOC);

                                foreach($registros as $option){
                                    ?>
                                    <option value="<?php echo $option['nm_titulo'] ?>"><?php echo $option['nm_titulo'] ?></option>
                                    <?php
                                }
                                ?>
                        ...
                        <div class="form-group col-sm-6 flex-column d-flex"> 
                            ...
                            <select name="nome" class="input"  required>
                            <option value="">Selecione o usuario</option>
                               <?php
                                $query = $conn->query("SELECT cd_usuario, nm_nome_usuario FROM tb_usuario ORDER BY nm_nome_usuario ASC");
                                $registros = $query->fetchAll(PDO::FETCH_ASSOC);

                                foreach($registros as $option){
                                    ?>
                                    <option value="<?php echo $option['nm_nome_usuario'] ?>"><?php echo $option['nm_nome_usuario'] ?></option>
                                    <?php
                                }
                                ?>
                    ...
                    <div class="form-group col-sm-6 flex-column d-flex"> 
                            ...
                            <input type="text" id="nm_status" name="nm_status" class="input" > 
                        </div>
